...
namespace Module
{
    ...
    public class SEED
    {
        ...
        public static byte[] Encrypt(byte[] Data, byte[] seedKey)
        {
            ...
            byte[] OutData = AddPadding(Data, 16);
            uint[] RoundKey = new uint[32];

            int nOutSize = (int)((OutData.Length) / 16) * 16;
            int nInSize = OutData.Length;
        ...
        public static byte[] Decrypt(byte[] Data, byte[] seedKey)
        {
            ...
            return DelPadding(OutData, 16);
        }

        private static byte[] AddPadding(byte[] iData, int blocksize)
        {
            int LastblockSize = iData.Length % blocksize;
            byte[] retData = null;
            if (LastblockSize != 0)
            {
                retData = new byte[iData.Length + (blocksize - LastblockSize)];
                Buffer.BlockCopy(iData, 0, retData, 0, iData.Length);
                int addCnt = blocksize - LastblockSize;
                for (int i = 0; i < addCnt; i++)
                {
                    retData[iData.Length + i] = 0x00;
                }
                retData[retData.Length - 1] = (byte)addCnt;
            }
            else
            {
                retData = iData;
        ...
        private static byte[] DelPadding(byte[] iData, int blocksize)
        {
            byte[] retData = null;
            bool PaddingChk = false;
            int lastValue = iData[iData.Length - 1];
            if (lastValue < (blocksize - 1))
            {
                int DelPaddingCount = lastValue - 1;

                for (int i = 2; i < (DelPaddingCount + 2); i++)
                {
                    if (iData[iData.Length - i] != 0x00)
                    {
                        PaddingChk = false;
                        break;
                    }
            ...
            if (PaddingChk)
            {
                retData = new byte[iData.Length - lastValue];
                Buffer.BlockCopy(iData, 0, retData, 0, retData.Length);
            }
            else
            {
                retData = iData;
        ...
        //암호화 함수
        public static string SeedEncoder(string iData)
        {
            byte[] Key = Encoding.UTF8.GetBytes("gkrdlvlvkdufthl.");
            //byte[] Key = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };
            byte[] EnData = Encrypt(Encoding.UTF8.GetBytes(iData), Key);
            //byte[] Key = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
            //byte[] tmpEnData = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F };
            //byte[] EnData = Encrypt(tmpEnData, Key);
            return Convert.ToBase64String(EnData);
        }
        //복호화 함수
        public static string SeedDecoder(string iData)
        {
            byte[] Key = Encoding.UTF8.GetBytes("gkrdlvlvkdufthl.");
            //byte[] Key = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };
            byte[] DeData = Decrypt(Convert.FromBase64String(iData), Key);

            return Encoding.UTF8.GetString(DeData);
        }
        //암호화 함수
        public static string SeedEncoder(string iData, string key)
        {
        ...
        //복호화 함수
        public static string SeedDecoder(string iData, string key)
        {
            byte[] Key = Encoding.UTF8.GetBytes(key);
            byte[] DeData = Decrypt(Convert.FromBase64String(iData), Key);
            return Encoding.UTF8.GetString(DeData);
